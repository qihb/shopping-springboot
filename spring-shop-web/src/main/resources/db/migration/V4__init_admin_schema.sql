-- Flyway 版本化迁移 V4：管理后台权限中心建表（spring-shop-admin）
-- 设计要点：
-- 1. 管理员（admin_user）与前台用户（user）分离，避免混用导致前台接口越权；
-- 2. RBAC 模型：admin_user - admin_user_role - role - role_menu - menu，
--    菜单即权限（含按钮级 permission_code），菜单表为树形结构；
-- 3. 操作日志（operation_log）记录管理员关键操作，供审计追溯。
